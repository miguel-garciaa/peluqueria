...
export function Hero({ onBook, onViewServices }: { onBook: () => void; onViewServices?: () => void }) {
  return (
      ...
      <div className="hero-content showcase-shell relative z-10 flex min-h-[100svh] flex-col items-center justify-center pb-20 pt-28 text-center">
        ...
        <div className="hero-aside mx-auto mt-8 max-w-2xl text-center">
          ...
          <div className="hero-actions mt-6 flex w-full flex-col items-center justify-center gap-3 sm:flex-row sm:items-stretch">
            ...
            <a href="/servicios" onClick={(event) => { if (!onViewServices || !window.matchMedia("(max-width: 47.999rem)").matches) return; event.preventDefault(); onViewServices(); }} className="w-[min(100%,21rem)] rounded-full border border-white/35 px-6 py-3 font-bold text-white transition-colors hover:bg-white hover:text-ink sm:w-auto sm:px-7 sm:py-4">Ver servicios</a>
          </div>
        </div>
      </div>
    </section>
