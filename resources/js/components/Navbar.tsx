...
function AccountControl({ currentUser, csrfToken, mobile = false }: NavbarProps & { mobile?: boolean }) {
    ...
    return (
      <details className={cn("group/account relative", mobile && "mt-4 w-full")}>
        ...
        <div className={cn("border border-white/10 bg-ink p-4 text-white", mobile ? "mt-3 rounded-2xl" : "absolute right-0 top-[calc(100%+0.75rem)] w-72 rounded-2xl shadow-2xl")}>
          ...
          <a href="/auth/google" className="mt-4 flex min-h-11 w-full items-center justify-center gap-2 rounded-md bg-white px-4 text-sm font-bold text-ink transition-colors hover:bg-brass focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brass">
  ...
  const logoutForm = <form action="/logout" method="post" className="mt-2"><input type="hidden" name="_token" value={csrfToken} /><button type="submit" className="flex min-h-10 w-full items-center justify-center gap-2 rounded-md bg-white/10 text-sm font-bold transition-colors hover:bg-white/15"><LogOut className="size-4" />Cerrar sesión</button></form>;
