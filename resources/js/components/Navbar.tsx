...
function AccountControl({ currentUser, csrfToken, mobile = false }: NavbarProps & { mobile?: boolean }) {
  ...
  const appointmentLink = <a href="/mis-citas" className="flex min-h-10 w-full items-center justify-center gap-2 rounded-full bg-white/10 text-sm font-bold transition-colors hover:bg-white/15"><CalendarDays className="size-4 text-brass" />Mis citas</a>;
  const adminLink = currentUser.isAdmin ? <a href="/admin" className="mb-2 flex min-h-10 w-full items-center justify-center gap-2 rounded-full bg-brass text-sm font-bold text-ink transition-colors hover:bg-brass-light"><LayoutDashboard className="size-4" />Panel de control</a> : null;
  ...
  if (mobile) {
    return <div className="mt-4 rounded-2xl border border-white/15 p-4"><div className="flex items-center gap-3">{currentUser.avatarUrl ? <img src={currentUser.avatarUrl} alt="" referrerPolicy="no-referrer" className="size-10 rounded-full object-cover" /> : <span className="grid size-10 place-items-center rounded-full bg-white/10"><UserRound className="size-5" /></span>}<div className="min-w-0"><p className="truncate font-bold">{currentUser.name}</p><p className="truncate text-xs text-white/55">{currentUser.email}</p></div></div><div className="mt-4">{adminLink}{appointmentLink}{logoutForm}</div></div>;
  }

  return <details className="group/account relative"><summary aria-label="Abrir cuenta" className="flex min-h-11 cursor-pointer list-none items-center gap-2 rounded-full border border-white/20 px-2.5 pr-4 text-sm font-bold text-white transition-colors hover:border-brass [&::-webkit-details-marker]:hidden">{currentUser.avatarUrl ? <img src={currentUser.avatarUrl} alt="" referrerPolicy="no-referrer" className="size-7 rounded-full object-cover" /> : <span className="grid size-7 place-items-center rounded-full bg-white/10"><UserRound className="size-4" /></span>}<span className="max-w-24 truncate">{currentUser.name.split(" ")[0]}</span></summary><div className="absolute right-0 top-[calc(100%+0.75rem)] w-64 rounded-2xl border border-white/10 bg-ink p-4 shadow-2xl"><p className="truncate font-bold">{currentUser.name}</p><p className="mt-1 truncate text-xs text-white/55">{currentUser.email}</p><div className="mt-3 border-t border-white/10 pt-3">{adminLink}{appointmentLink}{logoutForm}</div></div></details>;
}

function BookingAction({ onBook, mobile = false }: { onBook?: () => void; mobile?: boolean }) {
  const className = cn("navbar-cta group flex items-center gap-3 rounded-full bg-white p-2 pr-5 font-bold transition-transform hover:-translate-y-0.5", mobile ? "mt-4 justify-center" : "text-[0.95rem]");
  const content = <><span className={cn("grid place-items-center rounded-full bg-brass-deep text-white", mobile ? "size-10" : "size-9")}><ArrowUpRight className="size-4 transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" /></span><span className="whitespace-nowrap text-ink">Reservar cita</span></>;
