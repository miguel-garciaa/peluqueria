      ...
      <div className="container-shell">
        ...
        <ScrollReveal className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {displayedServices.map((service) => { const Icon = service.icon; return (
            <article key={service.id} className="min-h-[25rem]">
              <button type="button" onClick={() => setActiveService(service)} aria-label={`Ver detalles de ${service.title}`} className="group flex h-full w-full flex-col overflow-hidden rounded-2xl bg-white text-left text-ink ring-1 ring-ink/8 outline-none transition-[background-color,color,transform] duration-300 hover:-translate-y-1 hover:bg-ink hover:text-white focus-visible:ring-2 focus-visible:ring-brass-deep focus-visible:ring-offset-4 focus-visible:ring-offset-porcelain active:translate-y-0">
                <div className="relative h-40 w-full shrink-0 overflow-hidden bg-mist sm:h-44 lg:h-40 xl:h-44">
                  <img src={service.imageSrc} alt={service.imageAlt} loading="lazy" decoding="async" className="h-full w-full object-cover transition-transform duration-500 ease-out group-hover:scale-105" />
                  <div className="absolute inset-0 bg-gradient-to-t from-ink/45 via-transparent to-transparent" aria-hidden="true" />
                  <span className="absolute bottom-4 left-5 grid size-11 place-items-center rounded-full bg-white/92 text-brass-deep shadow-sm backdrop-blur-sm transition-colors group-hover:bg-brass group-hover:text-ink"><Icon className="size-5" aria-hidden="true" /></span>
                </div>
                <div className="flex min-h-0 flex-1 flex-col p-6 lg:p-5">
          ...
          return (
            <div className="service-dialog-layout relative grid h-full bg-porcelain">
              ...
              <div className="service-dialog-visual relative min-h-0 overflow-hidden bg-charcoal">
                ...
                <p className="absolute left-4 top-4 rounded-md bg-ink/78 px-3 py-2 text-xs font-semibold text-white backdrop-blur-sm md:bottom-8 md:left-8 md:top-auto">Asesoramiento incluido</p>
