      ...
      <form onSubmit={submit} className="flex h-full min-h-0 flex-col bg-porcelain">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-6 sm:px-7 sm:py-7">
          {success ? <div className="mx-auto flex min-h-full max-w-lg flex-col items-center justify-center py-10 text-center">
            ...
            <p className="mt-5 rounded-md bg-mist px-4 py-2 font-mono text-xs font-bold">Ref. {success.reference}</p>
            <div className="mt-8 flex w-full flex-col gap-3 sm:flex-row sm:justify-center"><a href="/mis-citas" className="rounded-md bg-ink px-6 py-3.5 font-bold text-white">Ver mis citas</a><button type="button" onClick={onClose} className="rounded-md border border-ink/15 px-6 py-3.5 font-bold">Cerrar</button></div>
          </div> : <>
            ...
            {step === 2 && <section aria-labelledby="step-service-title">
              ...
              <fieldset className="mt-6"><legend className="text-sm font-bold">¿Quién quieres que te atienda?</legend><div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => update("professionalId", "any")} aria-pressed={form.professionalId === "any"} className={cn("rounded-md border border-ink/15 bg-white px-4 py-3 text-sm font-bold", form.professionalId === "any" && "border-brass-deep bg-brass text-ink")}>Primera disponibilidad</button>{eligibleProfessionals.map((professional) => <button key={professional.id} type="button" onClick={() => update("professionalId", professional.id)} aria-pressed={form.professionalId === professional.id} className={cn("rounded-md border border-ink/15 bg-white px-4 py-3 text-sm font-bold", form.professionalId === professional.id && "border-ink bg-ink text-white")}>{professional.name.split(" ")[0]}</button>)}</div>{errors.professionalId && <p className="mt-2 text-xs text-danger">{errors.professionalId}</p>}</fieldset>
            </section>}
              ...
              <fieldset className="mt-5">
                ...
                <div className="mt-3 grid gap-3 sm:grid-cols-2">
                  ...
                  <label className={cn("flex items-center gap-3 rounded-2xl border p-4", catalog.bizumEnabled ? "cursor-pointer border-ink/10 bg-white" : "cursor-not-allowed border-ink/5 bg-mist/70 opacity-70")}>
                    ...
                    <span className="min-w-0"><span className="flex flex-wrap items-center gap-2"><strong className="text-sm">Bizum</strong>{!catalog.bizumEnabled && <span className="rounded-md bg-ink px-2 py-0.5 text-[0.62rem] font-bold uppercase tracking-wide text-white">Próximamente</span>}</span><span className="mt-0.5 block text-xs leading-5 text-taupe">Pago online mediante Redsys.</span></span>
                  </label>
                </div>
        ...
        {!success && <footer className="booking-dialog-footer flex shrink-0 items-center justify-between gap-3 border-t border-ink/10 bg-white px-5 py-4 sm:px-7">
          <button type="button" onClick={() => step === 1 ? onClose() : setStep((current) => current - 1)} className="flex min-h-12 items-center gap-2 rounded-md px-4 font-bold text-ink transition-colors hover:bg-mist"><ArrowLeft className="size-4" />{step === 1 ? "Cancelar" : "Atrás"}</button>
          {step < 4 ? <button type="button" onClick={(event) => { event.preventDefault(); continueToNextStep(); }} className="flex min-h-12 items-center gap-3 rounded-md bg-ink px-6 font-bold text-white transition-transform hover:-translate-y-0.5">Continuar <ArrowRight className="size-4" /></button> : <button type="submit" disabled={submitting} className="flex min-h-12 items-center gap-3 rounded-md bg-brass px-6 font-bold text-ink transition-transform hover:-translate-y-0.5 disabled:opacity-60"><Check className="size-4" />{submitting ? "Confirmando…" : "Confirmar cita"}</button>}
        </footer>}
      </form>
    </dialog>
  );
