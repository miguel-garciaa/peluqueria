          ...
          {appointments.length === 0 ? (
            <div className="mt-10 grid min-h-80 place-items-center rounded-2xl border border-dashed border-ink/20 bg-white p-8 text-center">
              <div>
                ...
                <button type="button" onClick={() => setBookingOpen(true)} className="mt-6 inline-flex rounded-md bg-ink px-6 py-3.5 font-bold text-white">Reservar mi primera cita</button>
              </div>
            </div>
          ) : (
                  ...
                  <article key={appointment.reference} className="grid overflow-hidden rounded-2xl border border-ink/10 bg-white sm:grid-cols-[7.5rem_1fr]">
                    ...
                    <div className="p-5 sm:p-6">
                      <div className="flex items-start justify-between gap-4">
                        ...
                        <span className={`rounded-md px-3 py-1.5 text-xs font-bold ${appointment.status === "confirmed" ? "bg-mist text-espresso" : appointment.status === "cancelled" ? "bg-danger/10 text-danger" : "bg-ink/5 text-taupe"}`}>{statusLabels[appointment.status]}</span>
                      </div>
                      ...
                      <div className="mt-4 flex min-h-11 flex-wrap items-center justify-between gap-3">
                        ...
                        {appointment.canCancel && !isConfirming && (
                          <button
                            ...
                            className="inline-flex min-h-11 items-center gap-2 rounded-md px-4 text-sm font-bold text-danger transition-colors hover:bg-danger/10 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-danger"
                          >
                        ...
                        <div className="mt-3 rounded-xl bg-danger/8 p-4" role="group" aria-label="Confirmar anulación">
                          ...
                          <div className="mt-3 flex flex-wrap gap-2">
                            <form method="POST" action={appointment.cancelUrl}>
                              ...
                              <button type="submit" className="min-h-11 rounded-md bg-danger px-4 text-sm font-bold text-white transition-opacity hover:opacity-90 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-danger">Sí, anular</button>
                            </form>
                            <button type="button" onClick={() => setConfirmingReference(null)} className="min-h-11 rounded-md border border-ink/15 px-4 text-sm font-bold transition-colors hover:bg-ink/5 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ink">Mantener cita</button>
                          </div>
                        </div>
                      )}
                    </div>
