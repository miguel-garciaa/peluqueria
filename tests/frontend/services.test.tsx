...
describe("Services", () => {
  ...
  it("shows the service photo inside every card", () => {
    render(<Services onBook={vi.fn()} catalogServices={databaseServices} />);

    expect(screen.getByRole("img", { name: "Estilista diseñando un corte personalizado" })).toBeInTheDocument();
  });

  it("uses the photo managed from the service catalog", () => {
    render(<Services onBook={vi.fn()} catalogServices={[{
      ...databaseServices[0],
      imageUrl: "/storage/services/corte.webp",
    }]} />);

    expect(screen.getByRole("img", { name: "Fotografía del servicio Corte & Peinado" })).toHaveAttribute(
      "src",
      "/storage/services/corte.webp",
    );
  });
