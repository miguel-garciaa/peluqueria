...
describe("Gallery", () => {
  it("does not place the same photograph on both sides of the marquee seam", () => {
    const { container } = render(<Gallery />);
    const originalImages = Array.from(container.querySelectorAll(".gallery-marquee-group:not([aria-hidden]) img"));
    const firstImage = originalImages.at(0);
    const lastImage = originalImages.at(-1);

    expect(firstImage).toBeDefined();
    expect(lastImage).toBeDefined();
    expect(lastImage).not.toHaveAttribute("src", firstImage?.getAttribute("src"));
  });
